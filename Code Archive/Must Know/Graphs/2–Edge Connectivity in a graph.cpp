...
//perform DFS on the graph starting from vertex v and 
//find all bridges in the process
int DFS(Graph const& graph, int v, vector<bool> visited, vector<int>& arrival, int parent, int& time, set<Edge>& bridges)
{
	...
	for (int w : graph.adjList[v])
	{
		...
		else if (w != parent)
		{
			...
			//from v. note that as visited[u] is already true
			//arrival [u] is already defined
			t = min(t, arrival[w]);
		}
	}
...
void printEdges(set<Edge> const& edges)
{
